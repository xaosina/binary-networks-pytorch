...
from .selectors.samplers import TopKSampler, BinarySoftActivation
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.ModuleList([conv(inplanes, planes, kernel_size=1, bias=False) for _ in range(4)]) 
        self.bn1 = nn.ModuleList([norm(planes) for _ in range(4)]) #nn.ModuleList([ for _ in range(4)])
        ...
        self.bn2 = nn.ModuleList([norm(planes) for _ in range(4)])
        self.conv3 = nn.ModuleList([
            conv(
                planes, planes * self.expansion, kernel_size=1, bias=False
            ) for _ in range(4)
        ])
        self.bn3 = nn.ModuleList([norm(planes * self.expansion) for _ in range(4)])
        self.relu = nn.ReLU(inplace=True) ############
        self.downsample = downsample
        self.stride = stride

        self.scales = nn.ParameterList([nn.Parameter(torch.rand(1), requires_grad=True) for _ in range(4)])
    ...
    def forward(self, x):
        ...
        final_output = []
        for conv1, conv2, conv3, bn1, bn2, bn3, scale in zip(
                self.conv1, self.conv2, self.conv3, self.bn1, self.bn2, self.bn3, self.scales
            ):
            out = conv1(x)
            out = bn1(out)
            out = self.relu(out)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.inplanes = 64
        self.norm = (
            lambda planes, momentum=0.05: nn.BatchNorm2d(
                planes, momentum=momentum,
            )
