...
class LengthSet(IterableDataset):
    ...
    def __iter__(self):
        return (
            {"model_input": b["model_input"], "label": b["label"]}
            for b in iter(self.dataset)
        )
...
def fashionMNIST(data_path=None, batch_size=-1, workers=-1):
    transform = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))]
    )
...
def get_base_imagenet(batch_size=32, workers=4):
    return get_tiny_image_net(
        "/home/dev/data_main/imagenet", batch_size=batch_size, workers=workers
    )


def get_tiny_image_net(
    data_path="/home/dev/data_main/CORESETS/TinyImagenet/tiny-224",
    batch_size=32,
    workers=4,
):
...
def get_wds_set(
    data_path="/home/dev/data_main/CORESETS/TinyImagenet_wds",
    batch_size=32,
    workers=4,
):
    lenghts = {
...
def get_tiny_imagenet_wds(batch_size=32, workers=4):
    ...
    train_loader, val_loader = get_wds_set(
        "/home/dev/data_main/CORESETS/TinyImagenet_wds", batch_size, workers
    )
    return train_loader, val_loader
...
def get_imagenet_wds(batch_size=32, workers=4):
    ...
    # "/home/dev/data_main/imagenet_fast": [1281167, 50000]
    train_loader, val_loader = get_wds_set(
        "/home/dev/data_main/imagenet_shards", batch_size, workers
    )
    return train_loader, val_loader
...
def get_imagenet_wds_new(batch_size=32, workers=4):
    ...
    # "/home/dev/data_main/imagenet_fast": [1281167, 50000]
    train_loader, val_loader = get_wds_set(
        "/home/dev/data_main/imagenet_wds", batch_size, workers
    )
    return train_loader, val_loader
...
def get_pascal(batch_size=64, workers=10):
    ...
    test_dataset = VOCSegmentation("/home/dev/data_main/PASCAL", train=False)
...
def get_cifar10(batch_size=32, workers=10):
    normalize = transforms.Normalize(
    ...
    train_dataset = DictSet(
        datasets.CIFAR10(
            root="/home/dev/data_main/CIFAR10",
            train=True,
            transform=train_transform,
            download=True,
        )
