...
parser.add_argument(
    "--exp_name", metavar="exp_name", default="test", help="experiment name"
)

parser.add_argument("--gpu", metavar="gpu", default=None, help="gpu")

parser.add_argument(
    "-u", "--user", type=str, choices=["dima", "egor"], required=True
)

parser.add_argument("--seed", metavar="seed", default=None, help="seed")
...
def run_experiment(model, get_loaders, target):
    ...
    train_loader, test_loader = get_loaders(
        workers=new_conf.HARDWARE.WORKERS,
    ...
    metric = {"classification": "accuracy", "segmentation": "iou_pascal"}[
        new_conf.PROBLEM
    ]

    criterion, metrics = get_metrics_and_loss(loss, [metric], TARGET)
    ...
    trainer.set_model(
        model, {"main": filter(lambda p: p.requires_grad, model.parameters())}
    )
