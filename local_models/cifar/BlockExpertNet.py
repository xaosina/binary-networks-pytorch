...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if True: #not self.sampling.get("effective", False):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=100, rprelu=False, sampling={}):
        self.inplanes = 64
        self.num_bases = 4
        self.rprelu = rprelu
        self.sampling = sampling
        super(ResNet, self).__init__()
        self.first_conv = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=True) #don't quantize the first layer
